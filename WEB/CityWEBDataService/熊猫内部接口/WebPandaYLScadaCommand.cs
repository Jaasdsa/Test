...
{
    public enum WebPandaYLScadaCommandType
    {
        CollectAndSaveScadaSensors,
        InitSensorRealData,
    }

    public class WebPandaYLScadaCommand
    {
        // SCADA-压力监测点-web接入轮询执行实体类
        WebPandaYLScadaCommandType Type;
        PandaParam param;

        public static WebPandaYLScadaCommand CreateCollectAndSaveScadaSensors(PandaParam param)
        {
            return new WebPandaYLScadaCommand() { Type = WebPandaYLScadaCommandType.CollectAndSaveScadaSensors, param = param };
        }
        public static WebPandaYLScadaCommand CreateInitSensorRealData(PandaParam param)
        {
            return new WebPandaYLScadaCommand() { Type = WebPandaYLScadaCommandType.InitSensorRealData, param = param };
        }
        ...
        public void Execute()
        {
            try
            {
                lock (this)  //会被多线程调用注意安全
                {
                    switch (Type)
                    {
                        case WebPandaYLScadaCommandType.CollectAndSaveScadaSensors:
                            CollectAndSaveSnesors();
                            break;
                        case WebPandaYLScadaCommandType.InitSensorRealData:
                            InitSensorRealData();
                            break;
                    }
                }
        ...
        private List<YaLiDian> GetWebYaLiData()
        {
            ...
            HttpRequestUtil requestUtil = new HttpRequestUtil(ContentType.XWwwFromUrlencoded);
            List<YaLiDian> data = new List<YaLiDian>();
            requestUtil.CreateSyncPostHttpRequest(url, parameters, (successData) =>
            {
                pandaWEBData = ByteUtil.ToDeserializeObject<PandaWEBData>(successData);
                if (pandaWEBData == null)
                {
                    TraceManagerForWeb.AppendErrMsg("获取熊猫监测点数据接口失败:" + "返回的数据格式不正确");
                    data = null;
                    return;
                }
                if (pandaWEBData.code == "200")
                {
                    TraceManagerForWeb.AppendDebug("获取熊猫监测点数据接口成功");
                    try
                    {
                        data = ByteUtil.ToDeserializeObject<List<YaLiDian>>(pandaWEBData.data.ToString());
                    }
                    catch (Exception e)
                    {
                        data = null;
                        TraceManagerForWeb.AppendErrMsg("反序列化熊猫监测点数据接口数据失败:" + e.Message);
                    }
                    return;
                }
                else
                {
                    TraceManagerForWeb.AppendErrMsg("获取熊猫监测点数据接口失败:" + pandaWEBData.msg);
                    data = null;
                    return;
                }
                ...
                data = null;
                TraceManagerForWeb.AppendErrMsg("获取熊猫监测点数据接口失败" + failData);
            }, (doErrorData) => {
                data = null;
                TraceManagerForWeb.AppendErrMsg("处理熊猫监测点数据接口失败" + doErrorData);
            });
